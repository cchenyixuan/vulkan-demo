...
# BORDER (wall shell layers) is a parameter: 9 = the validated 3-D default (2*hdx+1), 4 = one support radius (h = 4 dx).
# Pass BORDER=4 to build the lean variants (suffix _b4); default keeps 9 for parity with the validated cases.
source ~/run/tools/env.sh 2>/dev/null; cd "$(dirname "$0")/.." 2>/dev/null || cd ~/run/vulkan-demo
BORDER=${BORDER:-9}; SUF=""; [ "$BORDER" != 9 ] && SUF="_b$BORDER"
