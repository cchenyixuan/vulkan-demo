...
### Single-GPU baseline — V0 reference vs V5 single-GPU mode (measured 2026-09-23/24)

One headless RTX 5090 (UUID-pinned), 2-D cavity 1M–32M, 3 interleaved trials per point, warmup 1000 + measured 2000–10000 steps, validation off, no timestamps, drift 0 on all 240 runs (two campaigns); card at its 600 W limit (~2707–2810 MHz SM) throughout. Data `logs/single_baseline_20260923_v5fix/` (post-fix, paper) and `logs/single_baseline_20260923/` (pre-fix), both mirrored under `docs/single_gpu_baseline_20260923/`; figure `docs/single_gpu_baseline_v0_vs_v5.png`, write-up `docs/single_gpu_baseline_v0_vs_v5.md`. Campaign `_run_single_baseline_campaign.py`, summary `_summarize_single_baseline.py`, per-kernel timing `_run_kernel_breakdown.py`. The 1M point is `cases/lid_driven_cavity_2d_gen` (same generator/physics family as 2M–32M); the retuned `lid_driven_cavity_2d` is listed as `1m_orig`.

| size | V0 sync (fps) | V0 2-in-flight | V5 sync | V5 2-in-flight | V5/V0 (2-in-flight) |
|---|---|---|---|---|---|
| 1M | 521.9 ± 0.1 | 545.9 ± 0.3 | 522.7 ± 1.4 | 548.7 ± 0.3 | 100.5% |
| 2M | 267.2 | 274.4 | 269.7 | 277.1 | 101.0% |
| 4M | 129.8 | 131.8 | 129.9 | 132.1 | 100.2% |
| 8M | 69.8 | 70.5 | 70.7 | 71.4 | 101.3% |
| 16M | 35.4 | 35.5 | 35.8 | 36.0 | 101.2% |
| 32M | 18.2 | 18.3 | 18.4 | 18.5 | 101.2% |

**Headline (after commit `f9c660e`):** the multi-GPU solver in single-GPU mode runs at **100.1–102.0% of V0** on every point 1M–32M (std ≤0.5%); only the retuned `1m_orig` case stays at 97.3%. Two frames in flight are worth +5% at 1M (both solvers) fading to +0.4% at 32M — the exposed CPU submit bubble is ~100–165 µs/step. Per-particle throughput 545–600 M particle-steps/s.

**What the gap was (pre-fix V5 = 95.6–97.4% of V0, flat in N):** per-kernel timestamps put it entirely in density (+5%) and force (+6–7%); `SHARING_MODE_CONCURRENT` was ruled out (`V5_CONCURRENT_BUFFERS=0` diagnostic: ≤1.2%). Swapping the voxel_id encodings between V0 and V5 swapped their numbers exactly. Root cause: V5's slab-friendly **x-slowest** `voxel_id` encoding makes y the fastest memory axis, but the three neighbor kernels kept V0's `dz→dy→dx(innermost)` nesting. Reordering to `dx→dz→dy(innermost)` (same neighbor set, different summation order) made V5 1–2% faster than V0; validated by drift 0, 2×5090 dual smoke with exact conservation, and the K=2 equivalence battery (ALL PASS). **Rule: the innermost neighbor loop must follow the fastest-varying axis of the voxel encoding.** Consequence: every pre-fix V5 number (single and dual, June–September) is systematically 3–4% low; re-measure before quoting η. K=4 chains on this Windows rig stall in the transport workers with old and new shaders alike (pre-existing, unrelated).
