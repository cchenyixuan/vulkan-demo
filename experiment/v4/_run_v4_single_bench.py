"""
_run_v4_single_bench.py — V4 single-GPU headless BENCHMARK runner.
...
threads, one submit + one fence wait per frame. See SphSimulatorV4.
...
Usage:
    .venv/Scripts/python.exe experiment/v4/_run_v4_single_bench.py \\
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="V4 single-GPU benchmark runner")
...
def main() -> int:
    ...
    print(f"[bench_v4_single] case={args.case}")
    print(f"[bench_v4_single] device={args.device}  max_steps={args.max_steps}  "
          f"warmup={args.warmup}")

    # load_case_v4 returns a degenerate CaseV4 with no ghost / no transport
    # (see feedback_loader_partition_decoupling.md — partition_v4 is the
    # sole producer of ghost segments, and we're bypassing it here).
    case = load_case_v4(args.case)
    if args.disable_pst:
        print("[bench_v4_single] use_pst overridden to False")
    ...
    ctx = VulkanContextV4.create(
        device_index=args.device, enable_validation=args.validation,
        application_name="sph_v4_bench_single")

    sim = SphSimulatorV4(ctx, case)
    ...
    sim.bench = bench
    print(f"[bench_v4_single] timestampPeriod={bench.ns_per_tick:.4f}ns")
    ...
    if args.bench_csv:
        ...
        print(f"[bench_v4_single] CSV → {csv_path}")
    ...
    try:
        ...
        if args.validate_split:
            print(f"[bench_v4_single] VALIDATION MODE: using split pipeline variants")
        ...
        print(f"[bench_v4_single] {args.max_steps} steps in "
        ...
        print(f"[bench_v4_single] final: alive={alive:,} "
              f"(expected {expected_total:,})")
        if alive != expected_total:
            print(f"[bench_v4_single] WARN: alive drift = "
