"""
_run_v4_dual_bench.py — V4 dual-GPU headless BENCHMARK runner.

Same bootstrap as _run_v4_dual.py with no viewer and no debug-log path, plus
...
ticks baked in), then per frame extracts:
  ...
  - worker memcpy µs (already recorded by transport_v4's worker thread)
...
Design note: this runner exists separate from _run_v4_dual.py / _viewer
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="V4 dual-GPU benchmark runner")
    ...
    p.add_argument("--weights", default="3.2,1.0",
                   help="partition weights (W_AMD, W_NV). Default 3.2,1.0 is "
                        "the V4 Path A+ optimum for cross-vendor AMD+NV cavity "
                        "1M (277 fps). Override for other hardware/cases.")
    p.add_argument("--max-steps", type=int, default=2000)
    p.add_argument("--pool-safety", type=float, default=None,
                   help="per-slab own_pool_size = ceil(slab_particles * FACTOR) "
...
def main() -> int:
    ...
    from experiment.v4.utils.bench_v4 import BenchTimer, compute_durations
    ...
    print(f"[bench_v4_dual] case={args.case}")
    print(f"[bench_v4_dual] device_a={args.device_a} device_b={args.device_b} "
          f"weights={weights}  max_steps={args.max_steps}  warmup={args.warmup}")

    global_case = load_case_v4(args.case)
    if args.disable_pst:
        print("[bench_v4_dual] use_pst overridden to False")
        global_case.numerics.use_pst = False

    expected_total = int(global_case.initial.positions.shape[0])

    slab0, slab1, _k_split = compute_dual_gpu_partition(
    ...
    sim_a.bench = bench_a
    sim_b.bench = bench_b
    print(f"[bench_v4_dual] timestampPeriod a={bench_a.ns_per_tick:.4f}ns  "
    ...
    if args.bench_csv:
        ...
        print(f"[bench_v4_dual] CSV → {csv_path}")

    aggregator = WindowAggregator(window_size=args.bench_window)

    try:
        with DualGpuOrchestratorV4(sim_a, sim_b,
            ...
            while orch.frame_count < args.max_steps:
                ...
                # Per-defrag migration time series (orchestrator snapshots it
                # just before each defrag resets migration_install_count).
                if "defrag_report" in record:
                    fr = record["defrag_frame"]
                    a, b = record["defrag_report"][0], record["defrag_report"][1]
                    print(f"[migration] frame {fr:>6}  interval "
            ...
            print(f"[bench_v4_dual] {args.max_steps} steps in "
            ...
            print(f"[bench_v4_dual] final: a={s_a['alive_particle_count']:,} "
                  f"b={s_b['alive_particle_count']:,} "
                  f"total={total:,} (expected {expected_total:,})")
            if total != expected_total:
                print(f"[bench_v4_dual] WARN: alive drift = "
                      f"{total - expected_total}", file=sys.stderr)

            # V4 pool-health watermark: own-pool occupancy peak (for data-driven
            # own_pool_size sizing). WARN if within 10% of the limit; ERROR if it
            # overflowed (drops migrants).
            for tag, sim, status in (("AMD/a", sim_a, s_a), ("NV/b", sim_b, s_b)):
                ...
                print(f"[bench_v4_dual] pool_health[{tag}]: "
                      f"peak_tail={ph['peak_tail_high_water']:,} / "
                      f"own_pool={ph['own_pool_size']:,}  "
                      f"used={ph['used_fraction']*100:.1f}%  "
                      f"margin={ph['free_margin']:,}  "
                      f"peak_migration={ph['peak_migration_count']:,}"
