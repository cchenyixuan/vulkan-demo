...
SIZES = ["8m", "10m", "14m", "16m"]            # 1m/4m/6m already measured
WEIGHTS = [1.6, 1.7, 1.8, 1.9, 2.0]
...
def single_fps(device: int, size: str):
    out = _run([_PYTHON, "experiment/v4/_run_v4_single_bench.py", "--device", str(device),
...
def dual(size: str, weight: float):
    out = _run([_PYTHON, "experiment/v4/_run_v4_dual_pipeline.py", "--device-a", "0", "--device-b", "1",
