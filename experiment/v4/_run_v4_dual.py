"""
_run_v4_dual.py — V4 dual-GPU headless runner.

Runs the full V4 dual-GPU pipeline (3-submit + worker threads + sync2 timeline
+ ghost transport) on a YAML case, prints per-N-step status, ends with a
defrag-validated alive count.

Usage:
    .venv/Scripts/python.exe experiment/v4/_run_v4_dual.py [options]
...
Options (all optional):
    ...
    --weights W0,W1      partition weights  (default: 3.2,1.0 — empirically
                         optimal for V4 Path A+ on AMD 7900 XTX + NV 4060 Ti
...
No GLFW / viewer here. For visualization a separate _run_v4_dual_viewer.py
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="V4 dual-GPU headless runner")
    ...
    p.add_argument("--weights", default="3.2,1.0",
                   help="partition weights (W_AMD, W_NV). Default 3.2,1.0 is "
                        "the V4 Path A+ optimum for cross-vendor AMD 7900 XTX "
...
def main() -> int:
    ...
    print(f"[run_v4_dual] case={args.case}")
    print(f"[run_v4_dual] device_a={args.device_a} device_b={args.device_b} "
          f"weights={weights}")

    global_case = load_case_v4(args.case)
    if args.disable_pst:
        print("[run_v4_dual] use_pst overridden to False")
