"""
transport_v4.py — V4 cross-GPU ghost transport: per-pathway worker thread.

V4 v1.0 backend = CPU-staged 3-hop (see docs/sph_v4_design.md §6 + §14.5):

    sender VRAM → sender host_staging → memcpy → receiver host_staging → receiver VRAM

The two vkCmdCopyBuffer hops are folded into SphSimulatorV4's phase_a (readback)
...
V4 v1.0 spawns 2 GhostMigrationWorker instances (one per pathway: A→B and
...
class GhostMigrationWorker:
    ...
    def __init__(
        self,
        source_sim: "SphSimulatorV4",
        dest_sim: "SphSimulatorV4",
    ...
    def stop(self) -> None:
        ...
        orchestrator can react. Deferred per docs/sph_v4_design.md §14.4
    ...
    def _run(self) -> None:
        ...
        try:
            while True:
                ...
                #     "backwards" relative to host's. Same hazard as V4.0's
