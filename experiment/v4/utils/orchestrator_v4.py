"""
orchestrator_v4.py — Cross-GPU orchestrator for V4's 3-submit-per-frame pattern.

Owns 2 SphSimulatorV4 (borrows, does not destroy) + 2 GhostMigrationWorker
...
(docs/sph_v4_design.md §11.3) — v1.0 uses static partition only.
...
class DualGpuOrchestratorV4:
    """V4 dual-GPU orchestrator. Sim ownership: borrows only."""

    def __init__(
        self,
        sim_a: "SphSimulatorV4",
        sim_b: "SphSimulatorV4",
        *,
        ...
        for w in self.workers:
            w.start()
        print(f"[OrchV4] workers started ({self.worker_a_to_b.label}, "
    ...
    def __enter__(self) -> "DualGpuOrchestratorV4":
        return self
    ...
    def bootstrap_all(self) -> None:
        ...
        print(f"[OrchV4] bootstrap ghost sync: bridged "
        ...
        print(f"[OrchV4] both sims bootstrapped + step cmds ready")
    ...
    def step(self) -> dict:
        ...
        for sim_idx, sim in enumerate(self.sims):
            ...
            for poll_iter in range(MAX_POLL_ITERS):
                ...
                print(f"[OrchV4 WATCHDOG] frame {n}: waiting sim_{('a','b')[sim_idx]} "
    ...
    # ========================================================================
    # Submit-ahead pipelined run (V4: depth>1 frame pipelining, §5.4)
    # ========================================================================
