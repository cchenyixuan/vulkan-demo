"""
partition_v4.py — V4 dual-GPU static 1D X-axis partition.

Input contract: a **degenerate** CaseV4 from ``case_loader_v4.load_case_v4``
...
Given that input, computes:
  ...
  - per-GPU slab CaseV4    (own column range + leading/trailing ghost +
...
V4 v1.0: 2 GPUs only, static partition (V3+ generalizes to N + dynamic).
"""
...
GHOST_THICKNESS = 1   # V4 v1.0: 1-voxel-thick ghost on the interior side


def _ghost_pool_size(case: CaseV4) -> int:
    """Worst-case pid-slot count for ONE direction's ghost pool.

    V4 v1.0 ghost is 1 voxel thick (``GHOST_THICKNESS``), so the ghost zone
...
def compute_k_split(global_case: CaseV4, weights: list[float]) -> int:
    ...
    """
    if len(weights) != 2:
        raise NotImplementedError("V4 v1.0 supports exactly 2 GPUs")
...
def _build_slab_case(
    ...
    """Build a per-GPU CaseV4 for slot 0 (leftmost) or slot 1 (rightmost).
...
def compute_dual_gpu_partition(
...
) -> tuple[CaseV4, CaseV4, int]:
    """Top-level entry: split global_case into two slab CaseV4 + return K_split.
    ...
    """
    # Contract check — input must be a degenerate slab from load_case_v4.
    ...
    print(f"[partition_v4] K_split = {k_split} / {grid_nx} "
    ...
    if pool_safety is None:
        ...
        print(f"[partition_v4] pool_safety={pool_safety}: "
