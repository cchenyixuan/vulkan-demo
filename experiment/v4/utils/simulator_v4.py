"""
simulator_v4.py — V4 per-GPU SPH simulator.

V4 differences vs V1 (see docs/sph_v4_design.md):
...
V4 is fully self-contained: no imports from experiment/v1/ or utils/sph/.
"""
...
from experiment.v4.utils.case_v4 import (
    CaseV4,
    KIND_FLUID,
)
...
class SphSimulatorV4:
    ...
    Lifecycle:
        ctx = VulkanContextV4.create(device_index=...)
        case = load_case_v4("cases/lid_driven_cavity_2d/case.yaml")
        with SphSimulatorV4(ctx, case) as sim:
    ...
    def __init__(self, ctx: VulkanContextV4, case: CaseV4) -> None:
        self.ctx = ctx
        self.case = case
        ...
        print(f"[SimV4] init complete on {ctx.device_name} "
    ...
    def __enter__(self) -> "SphSimulatorV4":
        return self
    ...
    #
    # See docs/sph_v4_design.md §6 (Path A+ overhaul) for the rationale and
    # parallelism diagram. Transition from V4.0's 3N timeline (which had only
    ...
    def value_worker_done(self, frame_n: int) -> int:
        """Worker host-signals this after CPU memcpy completes. (Replaces V4.0's
    ...
    def _build_buffer_specs(self) -> list[_BufferSpec]:
        ...
        return [
            ...
            # binding 1: V4 pool-health watermark (4 uint = 16 B). Reclaims the
            # former unused overflow_log ring. See common.glsl PoolHealthBuffer.
            _BufferSpec("pool_health",                  3, 1,  16,                      BSU | TRANSFER),
            # V4 cleanup: bindings 2-6 (inlet_template / dispatch_indirect /
    ...
    def _allocate_buffer(
        ...
        (fallback path in VulkanContextV4), CONCURRENT degenerates to a
    ...
    def _allocate_buffers(self) -> dict[str, _Buffer]:
        ...
        for spec in self._buffer_specs:
            ...
            if shared:
                concurrent_count += 1
        print(f"[SimV4] device-local buffers: {len(buffers)}, "
    ...
    def _allocate_scratch_buffers(self) -> dict[str, _Buffer]:
        ...
        for spec in self._buffer_specs:
            ...
            total += spec.size
        print(f"[SimV4] defrag scratch buffers: {len(scratch)}, "
              f"{total / (1024 * 1024):.2f} MB")
        return scratch
    ...
    def _allocate_staging_buffers(self) -> dict[str, _Buffer]:
        ...
        — see docs/sph_v4_design.md §14.5. Persistent-mapped at construction.
        ...
        if stagings:
            ...
            print(f"[SimV4] host staging buffers: {len(stagings)}, "
            ...
            print(f"[SimV4] host staging buffers: 0 (single-GPU mode, no peer)")
        return stagings
    ...
    def _build_descriptor_layouts(self) -> list:
        """4 layouts: set 0/1/3 hold buffers; set 2 is empty (V4 merged-buffer
        ...
        for shader_name in (
            ...
            if not spv_path.exists():
                raise FileNotFoundError(
                    ...
                    f"Run experiment/v4/compile_shaders_v4.py first.")
    ...
    def _build_compute_pipelines(self) -> dict[str, object]:
        """Build the compute pipelines:
            ...
            3 × correction (ALL, INTERIOR, BOUNDARY)             ← V4 split
        ...
        # correction × 3 modes (V4 #1 — boundary band = 2 voxels)
        ...
        print(f"[SimV4] compute pipelines: {len(pipelines)}")
        return pipelines
    ...
    def _build_initial_data(self) -> dict[str, bytes]:
        ...
        V4 layout: own particles start at own_first_pid (V1.0a merged-buffer
    ...
    def _upload_initial_state(self) -> None:
        ...
        for buf in self.scratch_buffers.values():
            self._zero_buffer(buf)
        print(f"[SimV4] uploaded initial state ({len(initial)} payload buffers)")
    ...
    def bootstrap_compute(self) -> None:
        ...
        print(f"[SimV4] bootstrap done: alive={status['alive_particle_count']} "
    ...
    def bootstrap(self) -> None:
        ...
        if self._transport_segments:
            raise RuntimeError(
                ...
                "DualGpuOrchestratorV4.bootstrap_all()")
        self.bootstrap_init()
    ...
    #
    # V4 v1.0 single-GPU simplification: phase A only does predict + update_voxel;
    ...
    def _record_phase_a_cmd(self):
        """V4 phase A: predict + update_voxel + (per-direction: reset ghost_send_count
    ...
    def _record_phase_b_cmd(self):
        """V4 Path A+ Phase B: correction_interior + density_deep_interior over
        ...
        hidden. See docs/sph_v4_design.md Path A+ section for the cascading-
    ...
    def _record_phase_c_cmd(self):
        """V4 Phase C: per-direction upload + install_migration → correction
        ...
        See docs/sph_v4_design.md §5.3 + §7."""
        cmd = self._allocate_oneshot_cmd()
        vkBeginCommandBuffer(cmd, VkCommandBufferBeginInfo(
            flags=VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT))
    ...
    def prepare_step_cmd_buffers(self) -> None:
        ...
        print(f"[SimV4] step cmd buffers recorded "
    ...
    def prepare_step_single_cmd_buffer(self) -> None:
        ...
        print(f"[SimV4] step cmd buffer recorded (single-GPU combined)")
    ...
    def submit_step_single_and_wait(self) -> None:
        ...
        sync. Used by _run_v4_single_bench.py and any future single-GPU
        runner."""
    ...
    def _record_defrag_cmd(self):
        """V4 defrag cmd buffer (mirrors V1 _record_defrag_cmd):
