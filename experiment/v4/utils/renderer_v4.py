"""
renderer_v1.py — V1.0a fork of utils/sph/renderer.py for SphSimulatorV4.
...
Attaches to an SphSimulatorV4 without modifying it.
...
from experiment.v4.utils.camera_v4 import Camera
...
# V4 has its own compiled render SPV (identical shader source as V0/V1).
# parents[1] from experiment/v4/utils/renderer_v4.py = experiment/v4/.
SHADER_RENDER_DIR = (
...
class SphRendererV4:
    ...
    Use:
        sim = SphSimulatorV4(ctx, slab_case, leading_ghost_pool_size=L, ...)
        sim.bootstrap()  (or dual-GPU bootstrap dance)
        viewer = SphRendererV4(sim, window_width=1280, window_height=720)
        viewer.run()
        viewer.destroy()
    """
        ...
        try:
            while not glfw.window_should_close(self.window):
                ...
                if not self.paused and step_fn is not None:
                    # V4: runner-provided step_fn drives the orchestrator
    ...
    def __enter__(self) -> "SphRendererV4":
        return self
