"""
case_loader_v4.py — load a YAML case + OBJ vertex files into CaseV4.

V4 isolation: minimal self-contained loader; does NOT import utils/sph/.
Replicates the bare-minimum subset of utils/sph/case.py that V4 needs:
  ...
  - Compose CaseV4 ready to hand to SphSimulatorV4

What is NOT replicated:
  - Closest-packing self-check (V0 case.py has it; V4 trusts caller)
  - Per-source mask filtering (Phase 4 introduces this for multi-GPU)
  - Schema version validation (V4 reads schema_version: 2 directly)
...
def load_case_v4(case_yaml_path: str | pathlib.Path) -> CaseV4:
    """Parse a V0/V1-style case.yaml into a CaseV4.

    Output contract: the returned CaseV4 is a **degenerate slab** that owns
    ...
    Two consumption modes downstream:
      ...
      single-GPU: feed the returned CaseV4 directly into SphSimulatorV4;
                  the simulator's per-direction ghost flow auto-skips because
                  ``case.transport.has_*_peer`` is False on both sides.

      dual-GPU:   hand it to ``partition_v4.compute_dual_gpu_partition`` which
                  splits this global case into per-slab CaseV4s with populated
                  ghost / transport fields.

    Ghost column synthesis is partition_v4's job, not this loader's.
    """
    ...
    total_loaded = 0
    print(f"[case_loader_v4] loading {case_dir.name}")
    for entry in geometry["particles"]:
    ...
    print(f"[case_loader_v4] total loaded: {total_loaded:,} particles")
    ...
    # partition_v4 is responsible for synthesizing per-slab ghost / transport.
    grid = GridLayout(
    ...
    print(f"[case_loader_v4] grid: dim={dim_tuple} origin={origin_tuple}")
