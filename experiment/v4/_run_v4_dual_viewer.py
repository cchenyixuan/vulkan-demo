"""
_run_v4_dual_viewer.py — V4 dual-GPU viewer (GLFW window renders ONE slab).

Drives the full V4 dual-GPU pipeline (3-submit per frame + worker threads +
sync2 timeline + ghost transport) via DualGpuOrchestratorV4; renders the
chosen slab's particles in a GLFW window via SphRendererV4.
...
Usage:
    .venv/Scripts/python.exe experiment/v4/_run_v4_dual_viewer.py [options]

Options:
    --case PATH          case.yaml path
    --device-a/-b N      physical device indices (default 0 / 1)
    --weights W0,W1      partition weights  (default: 3.2,1.0 — V4 Path A+
...
Hotkeys (handled by SphRendererV4): SPACE pause, 0..4 color modes,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="V4 dual-GPU viewer (renders one slab)")
...
def main() -> int:
    ...
    from experiment.v4.utils.renderer_v4 import SphRendererV4
    ...
    print(f"[run_v4_viewer] case={args.case} render_slot={args.render_slot}")
    ...
    print(f"[run_v4_viewer] GLFW required instance extensions: {glfw_exts}")
    ...
    if args.render_slot == 0:
        sim_a = SphSimulatorV4(ctx_render, slab0)
        sim_b = SphSimulatorV4(ctx_compute, slab1)
        render_sim = sim_a
    else:
        sim_a = SphSimulatorV4(ctx_compute, slab0)
        sim_b = SphSimulatorV4(ctx_render, slab1)
        render_sim = sim_b
            ...
            if args.debug_log:
                logger = DebugLogger(
                    ...
                    meta_extra={
                        ...
                        "runner": "_run_v4_dual_viewer",
                    },
                )

            try:
                renderer = SphRendererV4(
                    render_sim,
                ...
                print(f"[run_v4_viewer] final alive: "
