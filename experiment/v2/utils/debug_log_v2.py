"""
debug_log_v2.py — periodic CSV log + ckpt-style buffer snapshots for V2 sims.
...
_BUFFER_VIEWS: dict[str, _BufferView] = {
    ...
    # ---- Set 3: globals + transport + materials ---------------------------
    "global_status":               (np.uint32, lambda c: (16,)),
    "overflow_log":                (np.uint32, lambda c: (-1,)),   # ring buffer, flat
    "inlet_template":              (np.uint8,  lambda c: (-1,)),
    "dispatch_indirect":           (np.uint32, lambda c: (4,)),
    "ghost_out_packet":            (np.uint8,  lambda c: (-1,)),
    "ghost_in_staging":            (np.uint8,  lambda c: (-1,)),
    "diagnostic":                  (np.uint8,  lambda c: (-1,)),
...
class _SimVoxelRanges:
    ...
    @classmethod
    def from_sim(cls, sim: "SphSimulatorV2") -> "_SimVoxelRanges":
        case = sim.case
...
class DebugLogger:
    ...
    Args:
        ...
        sims: dict[label, SphSimulatorV2] — label appears in CSV + dir names.
    ...
    def __init__(
        ...
        self._closed = False
        print(f"[DebugLoggerV2] output={self.output_dir} "
    ...
    def _log_one_sim(self, label: str, sim: "SphSimulatorV2", frame_n: int) -> None:
    ...
    def _snapshot_one_sim(self, label: str, sim: "SphSimulatorV2",
                          frame_n: int) -> None:
        ...
        if self.snapshot_buffers is None:
            ...
            if missing:
                print(f"[DebugLoggerV2] WARN: snapshot_buffers unknown to sim "
                      f"{label}: {missing}")
        ...
        print(f"[DebugLoggerV2] snapshot sim_{label} frame={frame_n} "
    ...
    def _write_npz(self, path: pathlib.Path, arrays: dict[str, np.ndarray],
                   frame_n: int, label: str, sim: "SphSimulatorV2") -> None:
