"""
renderer_v1.py — V1.0a fork of utils/sph/renderer.py for SphSimulatorV2.
...
Attaches to an SphSimulatorV2 without modifying it.
...
from experiment.v2.utils.camera_v2 import Camera
...
# V2 has its own compiled render SPV (identical shader source as V0/V1).
# parents[1] from experiment/v2/utils/renderer_v2.py = experiment/v2/.
SHADER_RENDER_DIR = (
...
class SphRendererV2:
    ...
    Use:
        sim = SphSimulatorV2(ctx, slab_case, leading_ghost_pool_size=L, ...)
        sim.bootstrap()  (or dual-GPU bootstrap dance)
        viewer = SphRendererV2(sim, window_width=1280, window_height=720)
        viewer.run()
        viewer.destroy()
    """
        ...
        try:
            while not glfw.window_should_close(self.window):
                ...
                if not self.paused and step_fn is not None:
                    # V2: runner-provided step_fn drives the orchestrator
    ...
    def __enter__(self) -> "SphRendererV2":
        return self
