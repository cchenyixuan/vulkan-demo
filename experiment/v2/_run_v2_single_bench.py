"""
_run_v2_single_bench.py — V2 single-GPU headless BENCHMARK runner.
...
threads, one submit + one fence wait per frame. See SphSimulatorV2.
...
Usage:
    .venv/Scripts/python.exe experiment/v2/_run_v2_single_bench.py \\
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="V2 single-GPU benchmark runner")
...
def main() -> int:
    ...
    print(f"[bench_v2_single] case={args.case}")
    print(f"[bench_v2_single] device={args.device}  max_steps={args.max_steps}  "
          f"warmup={args.warmup}")

    # load_case_v2 returns a degenerate CaseV2 with no ghost / no transport
    # (see feedback_loader_partition_decoupling.md — partition_v2 is the
    # sole producer of ghost segments, and we're bypassing it here).
    case = load_case_v2(args.case)
    if args.disable_pst:
        print("[bench_v2_single] use_pst overridden to False")
    ...
    ctx = VulkanContextV2.create(
        device_index=args.device, enable_validation=args.validation,
        application_name="sph_v2_bench_single")

    sim = SphSimulatorV2(ctx, case)
    ...
    sim.bench = bench
    print(f"[bench_v2_single] timestampPeriod={bench.ns_per_tick:.4f}ns")
    ...
    if args.bench_csv:
        ...
        print(f"[bench_v2_single] CSV → {csv_path}")
    ...
    try:
        ...
        print(f"[bench_v2_single] {args.max_steps} steps in "
        ...
        print(f"[bench_v2_single] final: alive={alive:,} "
              f"(expected {expected_total:,})")
        if alive != expected_total:
            print(f"[bench_v2_single] WARN: alive drift = "
