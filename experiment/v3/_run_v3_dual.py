"""
_run_v3_dual.py — V3 dual-GPU headless runner.

Runs the full V3 dual-GPU pipeline (3-submit + worker threads + sync2 timeline
+ ghost transport) on a YAML case, prints per-N-step status, ends with a
defrag-validated alive count.

Usage:
    .venv/Scripts/python.exe experiment/v3/_run_v3_dual.py [options]
...
Options (all optional):
    ...
    --weights W0,W1      partition weights  (default: 3.2,1.0 — empirically
                         optimal for V3 Path A+ on AMD 7900 XTX + NV 4060 Ti
...
No GLFW / viewer here. For visualization a separate _run_v3_dual_viewer.py
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="V3 dual-GPU headless runner")
    ...
    p.add_argument("--weights", default="3.2,1.0",
                   help="partition weights (W_AMD, W_NV). Default 3.2,1.0 is "
                        "the V3 Path A+ optimum for cross-vendor AMD 7900 XTX "
...
def main() -> int:
    ...
    print(f"[run_v3_dual] case={args.case}")
    print(f"[run_v3_dual] device_a={args.device_a} device_b={args.device_b} "
          f"weights={weights}")

    global_case = load_case_v3(args.case)
    if args.disable_pst:
        print("[run_v3_dual] use_pst overridden to False")
    ...
    ctx_a = VulkanContextV3.create(
        ...
        application_name="sph_v3_dual_a",
    ...
    ctx_b = VulkanContextV3.create(
        ...
        application_name="sph_v3_dual_b",
        enable_shared_host_transport=True)
