"""
_run_v3_dual_bench.py — V3 dual-GPU headless BENCHMARK runner.

Same bootstrap as _run_v3_dual.py with no viewer and no debug-log path, plus
...
ticks baked in), then per frame extracts:
  ...
  - worker memcpy µs (already recorded by transport_v3's worker thread)
...
Usage:
    .venv/Scripts/python.exe experiment/v3/_run_v3_dual_bench.py \\
        --case cases/lid_driven_cavity_2d/case.yaml \\
        --max-steps 5000 \\
        --bench-csv logs/bench_001.csv \\
        --bench-window 500

Design note: this runner exists separate from _run_v3_dual.py / _viewer
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="V3 dual-GPU benchmark runner")
    ...
    p.add_argument("--weights", default="3.2,1.0",
                   help="partition weights (W_AMD, W_NV). Default 3.2,1.0 is "
                        "the V3 Path A+ optimum for cross-vendor AMD+NV cavity "
...
def main() -> int:
    ...
    print(f"[bench_v3_dual] case={args.case}")
    print(f"[bench_v3_dual] device_a={args.device_a} device_b={args.device_b} "
          f"weights={weights}  max_steps={args.max_steps}  warmup={args.warmup}")

    global_case = load_case_v3(args.case)
    if args.disable_pst:
        print("[bench_v3_dual] use_pst overridden to False")
    ...
    ctx_a = VulkanContextV3.create(
        ...
        application_name="sph_v3_bench_a",
    ...
    ctx_b = VulkanContextV3.create(
        ...
        application_name="sph_v3_bench_b",
        enable_shared_host_transport=True)

    sim_a = SphSimulatorV3(ctx_a, slab0)
    sim_b = SphSimulatorV3(ctx_b, slab1)
    ...
    sim_a.bench = bench_a
    sim_b.bench = bench_b
    print(f"[bench_v3_dual] timestampPeriod a={bench_a.ns_per_tick:.4f}ns  "
    ...
    if args.bench_csv:
        ...
        print(f"[bench_v3_dual] CSV → {csv_path}")
    ...
    try:
        with DualGpuOrchestratorV3(sim_a, sim_b,
            ...
            print(f"[bench_v3_dual] {args.max_steps} steps in "
            ...
            print(f"[bench_v3_dual] final: a={s_a['alive_particle_count']:,} "
                  f"b={s_b['alive_particle_count']:,} "
                  f"total={total:,} (expected {expected_total:,})")
            if total != expected_total:
                print(f"[bench_v3_dual] WARN: alive drift = "
                      f"{total - expected_total}", file=sys.stderr)

            # V3 pool-health watermark: report the own-pool occupancy peak so the
            # static own_pool_size can be sized from data. WARN if the run got
            # within 10% of the limit; ERROR if it overflowed (drops migrants).
            for tag, sim, status in (("AMD/a", sim_a, s_a), ("NV/b", sim_b, s_b)):
                ph = sim.readback_pool_health()
                pct = ph["used_fraction"] * 100.0
                flag = ("  *** OVERFLOWED ***" if ph["free_margin"] <= 0
                        else "  ** WARN <10% margin **" if ph["used_fraction"] > 0.9
                        else "")
                print(f"[bench_v3_dual] pool_health[{tag}]: "
                      f"peak_tail={ph['peak_tail_high_water']:,} / "
                      f"own_pool={ph['own_pool_size']:,}  used={pct:.1f}%  "
