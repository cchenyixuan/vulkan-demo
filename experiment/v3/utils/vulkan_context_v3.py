"""
vulkan_context_v3.py — V3-internal Vulkan platform layer.

V3-specific differences vs V0/V1's utils/sph/vulkan_context.py:

  - apiVersion = 1.3  (V0 used 1.2). 1.3 promotes sync2 / vkQueueSubmit2 to
    core, so V3 can call them without enabling VK_KHR_synchronization2 as an
  ...
  - find_memory_type accepts (required, preferred) so HOST_CACHED can degrade
    ...
    cf. docs/sph_v3_design.md §14.5).

  - Per V3 isolation: this module does NOT import utils/sph/. Implementation
...
# V3 uses Vulkan 1.3 to get sync2 + timeline as core (no extension dance).
TARGET_API_VERSION = VK_MAKE_VERSION(1, 3, 0)

# Extensions required to participate in shared_host_transport_v3 (V3.3).
# Auto-included when VulkanContextV3.create(enable_shared_host_transport=True).
# Kept in sync with shared_host_transport_v3.REQUIRED_*_EXTENSIONS; duplicated
# here to avoid a module-level import cycle.
_SHARED_HOST_TRANSPORT_INSTANCE_EXTENSIONS = (
    "VK_KHR_get_physical_device_properties2",
    "VK_KHR_external_memory_capabilities",
    "VK_KHR_external_semaphore_capabilities",
)
_SHARED_HOST_TRANSPORT_DEVICE_EXTENSIONS = (
...
def _find_transfer_queue_family(
    ...
    import sys as _sys
    print(f"[VulkanContextV3] WARNING: no dedicated transfer queue family; "
...
def _select_physical_device(instance, requested_device_index: Optional[int]):
    ...
    print(f"[VulkanContextV3] available physical devices ({len(physical_devices)}):")
    ...
    print(f"[VulkanContextV3] selected: [{selected_index}] {selected_props.deviceName} "
...
@dataclass
class VulkanContextV3:
    """V3 compute-only Vulkan session handle bundle.

    Use ``VulkanContextV3.create(...)``; destroy via ``destroy()`` or
    context manager.
    ...
    def create(
        cls,
        application_name: str = "sph_v3",
        enable_validation: bool = True,
        extra_instance_extensions: Optional[list] = None,
        extra_device_extensions: Optional[list] = None,
        device_index: Optional[int] = None,
        enable_shared_host_transport: bool = False,
    ) -> "VulkanContextV3":
        """Create a V3 Vulkan context.

        ``enable_shared_host_transport``: if True, auto-include the instance
        and device extensions required by ``shared_host_transport_v3``
        (VK_KHR_external_memory_* + VK_KHR_external_semaphore_* +
        VK_EXT_external_memory_host). Required when this context will be
        used as one side of a SharedHostTransport in V3.3. Default False to
        keep Path A+ (V2/V3 baseline) callers unaffected; missing extensions
        are reported as a warning rather than raised so a single-GPU smoke
        run on hardware lacking these extensions still proceeds, but
        SharedHostTransport.create() will then fail at the entry-point
        lookup stage with a clear diagnostic.
        """
        # ---- Validation availability ------------------------------------
        validation_supported = _check_validation_layer_support()
        if enable_validation and not validation_supported:
            print(f"[VulkanContextV3] WARNING: validation layer requested but "
        ...
        if enable_shared_host_transport:
            # Avoid duplicates if caller already passed any of these via
            # extra_instance_extensions.
            for extension_name in _SHARED_HOST_TRANSPORT_INSTANCE_EXTENSIONS:
        ...
        print(f"[VulkanContextV3] transfer queue family: "
        ...
        # ---- Logical device ---------------------------------------------
        # V3 must enable BOTH timelineSemaphore (1.2 core feature) and
        ...
        # V3 render pipeline (added Phase 5+) doesn't need a feature dance.
        ...
        if enable_shared_host_transport:
            available_device_extensions = {
                p.extensionName for p in
                vkEnumerateDeviceExtensionProperties(physical_device, None)
            }
            missing = [
                extension_name
                for extension_name in _SHARED_HOST_TRANSPORT_DEVICE_EXTENSIONS
                if extension_name not in available_device_extensions
            ]
            if missing:
                print(
                    f"[VulkanContextV3] WARNING: enable_shared_host_transport=True "
                    f"but device '{device_name}' is missing required extensions: "
                    f"{missing}. SharedHostTransport.create() will fail. "
                    f"Continuing so unrelated callers still work.",
                    file=sys.stderr)
            for extension_name in _SHARED_HOST_TRANSPORT_DEVICE_EXTENSIONS:
                if (extension_name in available_device_extensions
                        and extension_name not in device_extension_list):
                    device_extension_list.append(extension_name)
    ...
    def __enter__(self) -> "VulkanContextV3":
        return self
