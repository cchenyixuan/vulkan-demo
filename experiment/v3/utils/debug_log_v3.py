"""
debug_log_v3.py — periodic CSV log + ckpt-style buffer snapshots for V3 sims.
...
Usage:
    ...
    with DualGpuOrchestratorV3(sim_a, sim_b, ...) as orch:
        orch.bootstrap_all()
...
_BUFFER_VIEWS: dict[str, _BufferView] = {
    ...
    "pool_health":                 (np.uint32, lambda c: (4,)),    # V3 watermark (reclaims overflow_log)
    # V3 cleanup: inlet_template / dispatch_indirect / ghost_out_packet /
...
class _SimVoxelRanges:
    ...
    @classmethod
    def from_sim(cls, sim: "SphSimulatorV3") -> "_SimVoxelRanges":
        case = sim.case
...
class DebugLogger:
    ...
    Args:
        ...
        sims: dict[label, SphSimulatorV3] — label appears in CSV + dir names.
    ...
    def __init__(
        ...
        self._closed = False
        print(f"[DebugLoggerV3] output={self.output_dir} "
    ...
    def _log_one_sim(self, label: str, sim: "SphSimulatorV3", frame_n: int) -> None:
    ...
    def _snapshot_one_sim(self, label: str, sim: "SphSimulatorV3",
                          frame_n: int) -> None:
        ...
        if self.snapshot_buffers is None:
            ...
            if missing:
                print(f"[DebugLoggerV3] WARN: snapshot_buffers unknown to sim "
                      f"{label}: {missing}")
        ...
        print(f"[DebugLoggerV3] snapshot sim_{label} frame={frame_n} "
    ...
    def _write_npz(self, path: pathlib.Path, arrays: dict[str, np.ndarray],
                   frame_n: int, label: str, sim: "SphSimulatorV3") -> None:
