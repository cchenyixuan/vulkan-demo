"""
simulator_v3.py — V3 per-GPU SPH simulator.

V3 differences vs V1 (see docs/sph_v3_design.md):
...
V3 is fully self-contained: no imports from experiment/v1/ or utils/sph/.
"""
...
if TYPE_CHECKING:
    from experiment.v3.utils.shared_host_transport_v3 import (
...
# ============================================================================
# Constants — descriptor binding layout (matches shaders/v3/common.glsl)
# ============================================================================
...
class SphSimulatorV3:
    ...
    Lifecycle:
        ctx = VulkanContextV3.create(device_index=...)
        case = load_case_v3("cases/lid_driven_cavity_2d/case.yaml")
        with SphSimulatorV3(ctx, case) as sim:
    ...
    def __init__(self, ctx: VulkanContextV3, case: CaseV3) -> None:
        self.ctx = ctx
        self.case = case
        ...
        # V3.3: staging is no longer per-sim. _allocate_staging_buffers now
        # computes per-direction transport segments + total bytes (needed by
        # the orchestrator to size SharedHostTransport) but does NOT allocate
        # any host RAM. For single-GPU sims this remains a no-op (no peer
        # directions); for dual-GPU sims the orchestrator must call
        # attach_shared_transport() with a populated SharedHostTransport before
        # bootstrap_init so cmd recording can use the shared region buffers.
        self.staging_buffers: dict[str, _Buffer] = self._allocate_staging_buffers()

        # V3.3 cross-device transport state (populated by attach_shared_transport).
        # When this sim has a peer (i.e. self._transport_segments non-empty),
        # both attributes are mandatory before bootstrap_init / prepare_step_
        # cmd_buffers can run. When this sim has no peer (single-GPU mode),
        # both remain None / empty forever.
        self.shared_host_transport: Optional["SharedHostTransport"] = None
        self.transport_side: Optional[str] = None
        # Per-direction binary cross-device semaphores. Phase A signals each
        # outgoing semaphore on submission; Phase C waits on each incoming
        # semaphore on submission. For the 2-GPU case each sim has exactly one
        # peer direction so each dict has at most one entry.
        self.outgoing_signal_semaphores: dict[str, object] = {}
        self.incoming_wait_semaphores: dict[str, object] = {}
        ...
        # V3.3: transfer queue cmds removed. In Path A+ each direction had
        # a separate readback cmd (transfer Q) and upload cmd (transfer Q)
        # bridged by a CPU worker memcpy; with shared host RAM the readback
        # vkCmdCopyBuffer is recorded inline at the end of phase_a_cmd
        # (compute Q) and the upload vkCmdCopyBuffer at the head of
        # phase_c_cmd (compute Q), eliminating the worker, the transfer
        # queue submissions, and the cross-family timeline handshake that
        # caused NV correction_interior to regress +50-76%.
        ...
        print(f"[SimV3] init complete on {ctx.device_name} "
    ...
    def destroy(self) -> None:
        ...
        # V3.3: staging buffers are NOT owned by the sim — they alias
        # SharedHostTransport's per-device VkBuffer (which is owned by the
        # orchestrator-held SharedHostTransport and destroyed there). Just
        # drop our references so any later sim use raises AttributeError.
        self.staging_buffers = {}
        self.shared_host_transport = None
        self.outgoing_signal_semaphores = {}
        self.incoming_wait_semaphores = {}
    ...
    def __enter__(self) -> "SphSimulatorV3":
        return self
    ...
    #
    # See docs/sph_v3_design.md §6 (Path A+ overhaul) for the rationale and
    # parallelism diagram. Transition from V3.0's 3N timeline (which had only
    ...
    def value_worker_done(self, frame_n: int) -> int:
        """Worker host-signals this after CPU memcpy completes. (Replaces V3.0's
    ...
    def _build_buffer_specs(self) -> list[_BufferSpec]:
        ...
        return [
            ...
            # binding 1: V3 pool-health watermark (4 uint = 16 B). Reclaims the
            # former unused overflow_log ring. See common.glsl PoolHealthBuffer.
            _BufferSpec("pool_health",                  3, 1,  16,                      BSU | TRANSFER),
            # V3 cleanup: bindings 2-6 (inlet_template / dispatch_indirect /
    ...
    def _allocate_buffer(
        ...
        (fallback path in VulkanContextV3), CONCURRENT degenerates to a
    ...
    def _allocate_buffers(self) -> dict[str, _Buffer]:
        ...
        for spec in self._buffer_specs:
            ...
            if shared:
                concurrent_count += 1
        print(f"[SimV3] device-local buffers: {len(buffers)}, "
    ...
    def _allocate_scratch_buffers(self) -> dict[str, _Buffer]:
        ...
        for spec in self._buffer_specs:
            ...
            total += spec.size
        print(f"[SimV3] defrag scratch buffers: {len(scratch)}, "
              f"{total / (1024 * 1024):.2f} MB")
        return scratch
    ...
    def _allocate_staging_buffers(self) -> dict[str, _Buffer]:
        """V3.3: compute per-direction transport segments + total bytes for
        SharedHostTransport sizing. Does NOT allocate any host RAM buffers
        — those live in SharedHostTransport and are attached via
        attach_shared_transport().

        Returns an empty dict in all modes. The dict is populated later by
        attach_shared_transport() for dual-GPU mode; it stays empty for
        single-GPU mode (no peer = no transport).

        History (worker path, removed): V3 baseline + V3 Path A+ allocated
        a CACHED sender_staging + COHERENT receiver_staging per direction
        (2 buffers × 6.4 MB on 4M cavity) and bridged them with a CPU
        worker thread memcpy. The worker-bridge architecture forced
        sender_staging into HOST_CACHED (no ReBAR alternative — see git
        blame for the B3 experiment that lost 23% fps when receiver was
        moved to ReBAR). V3.3 collapses both stagings into a single shared
        host buffer imported by both VkDevices, eliminating the worker
        memcpy and the architectural ReBAR limitation."""
        # Always populate these dicts — needed by cmd recording even when
        # the actual staging buffer handles are attached later.
        ...
        if self._transport_segments:
            print(f"[SimV3] transport segments computed for "
                  f"{list(self._transport_total_bytes)} "
                  f"({sum(self._transport_total_bytes.values()) / 1024:.1f} KB "
                  f"total; staging buffers attach later via "
                  f"attach_shared_transport)")
        else:
            print(f"[SimV3] no transport segments (single-GPU mode, no peer)")
        return {}

    # ========================================================================
    # V3.3 shared host transport attachment
    # ========================================================================

    def transport_bytes_for_direction(self, direction: str) -> int:
        """Returns per-direction transport size in bytes (0 if no peer in
        that direction). Used by the orchestrator to size
        SharedHostTransport regions before calling attach_shared_transport.
        """
        return self._transport_total_bytes.get(direction, 0)

    def attach_shared_transport(
        self,
        shared_host_transport: "SharedHostTransport",
        side: str,
    ) -> None:
        """Wire this sim into a SharedHostTransport. Must be called BEFORE
        bootstrap_init() / prepare_step_cmd_buffers() for any dual-GPU sim.
        For single-GPU sims (no peer) this is a no-op safety method.

        ``side`` ∈ {'a', 'b'} — which side of the SharedHostTransport this
        sim represents. For the standard 2-GPU 1D layout:
          - side='a' = leftmost sim, has trailing peer (sim_b is on the right)
                       outgoing region = a_to_b (this sim is src)
                       incoming region = b_to_a (this sim is dst)
          - side='b' = rightmost sim, has leading peer (sim_a is on the left)
                       outgoing region = b_to_a (this sim is src)
                       incoming region = a_to_b (this sim is dst)

        After this returns, self.staging_buffers contains stub _Buffer
        wrappers whose .handle points to the shared region's per-device
        VkBuffer (NOT owned by self — destroy() will not free them).
        Outgoing / incoming binary semaphore dicts are populated in
        parallel; submit_phase_a / submit_phase_c reference them.
        """
        if side not in ("a", "b"):
            raise ValueError(f"side must be 'a' or 'b'; got {side!r}")
        if self.shared_host_transport is not None:
            raise RuntimeError(
                "attach_shared_transport already called on this sim; "
                "re-attachment is not supported")
        if not self._transport_segments:
            # Single-GPU sim with no peer — attach is a no-op. Allowed so
            # the orchestrator can call attach unconditionally on both sims.
            return

        # Map this sim's per-direction (sender/receiver) staging onto
        # SharedHostTransport's per-region per-device VkBuffers.
        #
        # Naming sanity check: the only direction this sim should have on
        # a 2-GPU layout matches the side.
        expected_direction_for_side = {"a": "trailing", "b": "leading"}[side]
        actual_directions = list(self._transport_segments)
        if actual_directions != [expected_direction_for_side]:
            raise ValueError(
                f"side='{side}' implies the sim should have exactly the "
                f"'{expected_direction_for_side}' peer direction, but the "
                f"sim's case lists transport directions {actual_directions}. "
                f"Did you swap sim_a / sim_b at orchestrator construction?")
        direction = expected_direction_for_side

        if side == "a":
            outgoing_region = shared_host_transport.region_a_to_b
            incoming_region = shared_host_transport.region_b_to_a
            outgoing_semaphore = shared_host_transport.semaphore_a_to_b.src_semaphore
            incoming_semaphore = shared_host_transport.semaphore_b_to_a.dst_semaphore
        else:
            outgoing_region = shared_host_transport.region_b_to_a
            incoming_region = shared_host_transport.region_a_to_b
            outgoing_semaphore = shared_host_transport.semaphore_b_to_a.src_semaphore
            incoming_semaphore = shared_host_transport.semaphore_a_to_b.dst_semaphore

        # Sanity-check region sizes match what this sim computed.
        for region, role in ((outgoing_region, "outgoing"),
                              (incoming_region, "incoming")):
            if region.size < self._transport_total_bytes[direction]:
                raise ValueError(
                    f"{role} shared region size {region.size} < this sim's "
                    f"transport_total_bytes[{direction}]="
                    f"{self._transport_total_bytes[direction]}. "
                    f"Orchestrator must size regions from both sims' "
                    f"transport_bytes_for_direction() calls.")

        # Locate the per-device VkBuffer view inside each region (src for
        # outgoing where this device wrote, dst for incoming where this
        # device reads).
        if outgoing_region.src_view.device != self.ctx.device:
            raise RuntimeError(
                "outgoing region's src_view belongs to a different VkDevice "
                "than this sim's ctx — check side passed to "
                "attach_shared_transport")
        if incoming_region.dst_view.device != self.ctx.device:
            raise RuntimeError(
                "incoming region's dst_view belongs to a different VkDevice "
                "than this sim's ctx — check side passed to "
                "attach_shared_transport")

        sender_size = self._transport_total_bytes[direction]
        receiver_size = self._transport_total_bytes[direction]

        sender_stub = _Buffer(
            handle=outgoing_region.src_view.buffer,
            memory=None,                       # owned by SharedHostTransport
            size=sender_size,
            mapped=None,
            mapped_view=outgoing_region.mapped_view[:sender_size],
        )
        receiver_stub = _Buffer(
            handle=incoming_region.dst_view.buffer,
            memory=None,
            size=receiver_size,
            mapped=None,
            mapped_view=incoming_region.mapped_view[:receiver_size],
        )
        self.staging_buffers[f"sender_staging_{direction}"] = sender_stub
        self.staging_buffers[f"receiver_staging_{direction}"] = receiver_stub

        self.outgoing_signal_semaphores[direction] = outgoing_semaphore
        self.incoming_wait_semaphores[direction] = incoming_semaphore

        self.shared_host_transport = shared_host_transport
        self.transport_side = side

        print(f"[SimV3] attached SharedHostTransport (side={side}, "
              f"direction={direction}, sender/receiver = "
              f"{sender_size / 1024:.1f} KB shared host RAM)")
    ...
    def _build_descriptor_layouts(self) -> list:
        """4 layouts: set 0/1/3 hold buffers; set 2 is empty (V3 merged-buffer
        ...
        for shader_name in (
            ...
            if not spv_path.exists():
                raise FileNotFoundError(
                    ...
                    f"Run experiment/v3/compile_shaders_v3.py first.")
    ...
    def _build_compute_pipelines(self) -> dict[str, object]:
        """Build the compute pipelines:
            ...
            3 × correction (ALL, INTERIOR, BOUNDARY)             ← V3 split
        ...
        # correction × 3 modes (V3 #1 — boundary band = 2 voxels)
        ...
        print(f"[SimV3] compute pipelines: {len(pipelines)}")
        return pipelines
    ...
    # V3.3: _allocate_transfer_oneshot_cmd removed. Readback / upload DMAs
    # now run inline on the compute queue (phase A tail, phase C head); no
    # transfer-queue cmd buffers need to be allocated.
    ...
    def _build_initial_data(self) -> dict[str, bytes]:
        ...
        V3 layout: own particles start at own_first_pid (V1.0a merged-buffer
    ...
    def _upload_initial_state(self) -> None:
        ...
        for buf in self.scratch_buffers.values():
            self._zero_buffer(buf)
        print(f"[SimV3] uploaded initial state ({len(initial)} payload buffers)")
    ...
    def bootstrap_compute(self) -> None:
        ...
        print(f"[SimV3] bootstrap done: alive={status['alive_particle_count']} "
    ...
    def bootstrap(self) -> None:
        ...
        if self._transport_segments:
            raise RuntimeError(
                ...
                "DualGpuOrchestratorV3.bootstrap_all()")
        self.bootstrap_init()
    ...
    def readback_pool_health(self) -> dict:
        ...
        peak_tail, peak_migration, _r0, _r1 = struct.unpack("<I I I I", payload)
        own_pool = self.case.capacities.own_pool_size
        return {
    ...
    #
    # V3 v1.0 single-GPU simplification: phase A only does predict + update_voxel;
    ...
    def _record_phase_a_cmd(self):
        """V3.3 phase A: predict + update_voxel + (per-direction: reset
        ghost_send_count + ghost_send dispatch + readback vkCmdCopyBuffer
        to SharedHostTransport's outgoing region). Signals frame_done at
        submit; phase A also signals the per-direction cross-device binary
        semaphore so the peer's phase C can wait on it. All on compute
        queue (V3.3: no transfer queue).
        """
        cmd = self._allocate_oneshot_cmd()
        vkBeginCommandBuffer(cmd, VkCommandBufferBeginInfo(
        ...
        # Per-direction ghost flow (skipped if this GPU has no peer on that
        # side). V3.3: ghost_send.comp dispatch → vkCmdCopyBuffer device→
        # shared host RAM inline at the end of phase A on the compute queue.
        # Phase A's submission signals a cross-device binary semaphore that
        # the peer's phase C waits on; no transfer queue, no cross-family
        # handshake, no NV cache invalidation regression.
        for direction in ("leading", "trailing"):
            if direction not in self._transport_segments:
                continue
            self._record_compute_barrier(cmd)
            ...
            # V3.3 inline readback (was on transfer Q in Path A+).
            self._record_compute_to_transfer_barrier(cmd)
            self._record_readback_for_direction(cmd, direction)
            self._bench_tick(cmd, f"a_readback_{direction}_end")

        vkEndCommandBuffer(cmd)
        return cmd

    def _record_phase_b_cmd(self):
        """V3 Path A+ Phase B: correction_interior + density_deep_interior over
        ...
        hidden. See docs/sph_v3_design.md Path A+ section for the cascading-
    ...
    def _record_phase_c_cmd(self):
        """V3 Phase C: per-direction upload + install_migration → correction
        ...
        See docs/sph_v3_design.md §5.3 + §7."""
        cmd = self._allocate_oneshot_cmd()
        vkBeginCommandBuffer(cmd, VkCommandBufferBeginInfo(
            flags=VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT))
        ...
        # V3.3: upload vkCmdCopyBuffer runs INLINE at the head of phase C
        # on the compute queue, after the cross-device binary semaphore
        # wait (added in submit_phase_c). The semaphore wait guarantees
        # the peer's phase A readback DMA into the shared host RAM has
        # completed before this copy reads the same bytes back into our
        # local VRAM ghost region. No transfer queue, no NV cache
        # invalidation regression.
        for direction in ("leading", "trailing"):
            if direction not in self._transport_segments:
                continue
            # V3.3 inline upload (was on transfer Q in Path A+).
            self._record_upload_for_direction(cmd, direction)
            self._bench_tick(cmd, f"c_upload_{direction}_end")
            # Transfer→compute visibility barrier before install_migrations
            # reads the ghost-pid SoA we just wrote.
            self._record_transfer_to_compute_barrier(cmd)
            self._bind_pipeline_and_sets(cmd, f"install_migrations_{direction}")
            per_ghost_pid = self._per_ghost_pid_dispatch_count(direction)
            vkCmdDispatch(cmd, per_ghost_pid, 1, 1)
    ...
    # V3.3: _record_transfer_readback_cmd / _record_transfer_upload_cmd
    # removed. The vkCmdCopyBuffer they used to record on the transfer
    # queue is now appended at the end of _record_phase_a_cmd and
    # prepended at the start of the per-direction loop in
    # _record_phase_c_cmd, all on the compute queue. Cross-device sync
    # uses a binary semaphore signaled by phase A's submission and waited
    # by the peer's phase C submission.

    def prepare_step_cmd_buffers(self) -> None:
        """Record phase A/B/C cmd buffers once. Caller invokes after
        bootstrap. Re-call to re-record (e.g. when switching
        CORRECTION_MODE_ALL → split in Phase 6).

        V3.3: dual-GPU sims must have attach_shared_transport() called
        beforehand so phase A's readback and phase C's upload can address
        the shared region buffers. _record_phase_a_cmd /
        _record_phase_c_cmd both raise KeyError on staging_buffers lookup
        if the attach was skipped — that's intended fail-fast behavior."""
        device = self.ctx.device
        ...
        if self._transport_segments and self.shared_host_transport is None:
            raise RuntimeError(
                "prepare_step_cmd_buffers: this sim has peer directions "
                f"({list(self._transport_segments)}) but no SharedHostTransport "
                "attached. The orchestrator must call attach_shared_transport "
        ...
        print(f"[SimV3] step cmd buffers recorded "
              f"(phase A/B/C, {n_dir} peer direction(s) inline on compute Q)")
    ...
    def prepare_step_single_cmd_buffer(self) -> None:
        ...
        print(f"[SimV3] step cmd buffer recorded (single-GPU combined)")
    ...
    def submit_step_single_and_wait(self) -> None:
        ...
        sync. Used by _run_v3_single_bench.py and any future single-GPU
        runner."""
    ...
    def submit_with_timeline(
        ...
        *,
        ...
        extra_wait_binary_semaphores: Optional[list] = None,
        extra_wait_binary_stage: int = VK_PIPELINE_STAGE_2_ALL_TRANSFER_BIT,
        extra_signal_binary_semaphores: Optional[list] = None,
        extra_signal_binary_stage: int = VK_PIPELINE_STAGE_2_ALL_TRANSFER_BIT,
    ) -> None:
        """Submit ``cmd`` with timeline-semaphore wait/signal plus optional
        cross-device binary semaphores. Defaults to compute_queue +
        compute_shader stage mask.

        ``extra_wait_binary_semaphores`` / ``extra_signal_binary_semaphores``
        are V3.3 cross-device binary sems (VK_KHR_external_semaphore_win32
        OPAQUE_WIN32 imports). Phase C uses extra_wait to block on the peer's
        phase A readback completion; phase A uses extra_signal to signal the
        peer's phase C wait. Stage mask for these is ALL_TRANSFER because
        the gated work is vkCmdCopyBuffer on shared host RAM, not compute.
        """
        if queue is None:
        ...
        if extra_wait_binary_semaphores:
            for binary_sem in extra_wait_binary_semaphores:
                wait_infos.append(VkSemaphoreSubmitInfo(
                    sType=VK_STRUCTURE_TYPE_SEMAPHORE_SUBMIT_INFO,
                    semaphore=binary_sem,
                    value=0,  # ignored for binary semaphores
                    stageMask=extra_wait_binary_stage,
                ))
        signal_infos = []
        ...
        if extra_signal_binary_semaphores:
            for binary_sem in extra_signal_binary_semaphores:
                signal_infos.append(VkSemaphoreSubmitInfo(
                    sType=VK_STRUCTURE_TYPE_SEMAPHORE_SUBMIT_INFO,
                    semaphore=binary_sem,
                    value=0,
                    stageMask=extra_signal_binary_stage,
                ))
    ...
    def submit_phase_a(self, frame_n: int) -> None:
        """Compute Q: predict + update_voxel + ghost_send + (V3.3) inline
        readback vkCmdCopyBuffer device → shared host RAM.

        Waits prev frame_done on timeline. Signals 5N+1 on timeline AND
        each outgoing cross-device binary semaphore (the peer's phase C
        will wait on these). The signal-after-all-recorded-work semantics
        guarantee the readback DMA has completed before the peer reads
        the shared host RAM.
        """
        ...
        outgoing_sems = list(self.outgoing_signal_semaphores.values())
        self.submit_with_timeline(
            self.phase_a_cmd,
            wait_value=wait_value,
            signal_value=self.value_phase_a_done(frame_n),
            extra_signal_binary_semaphores=outgoing_sems if outgoing_sems else None,
        )
    ...
    def submit_phase_b(self, frame_n: int) -> None:
        ...
        V3.3: phase B no longer overlaps a transfer-queue chain (which
        doesn't exist anymore). It still benefits from being submitted as
        a separate cmd buffer because the driver's queue scheduler can
        start B's first dispatch as soon as ghost_send.comp finishes,
        without waiting for the inline readback DMA at the tail of phase A.
        On AMD this nearly hides the readback DMA behind correction_
        interior anyway (cmd-buffer-level parallelism)."""
    ...
    def submit_phase_c(self, frame_n: int) -> None:
        """Compute Q: (V3.3 inline upload from shared host RAM) +
        install_migrations + correction_boundary + density + force.

        Waits on each incoming cross-device binary semaphore (peer's phase
        A signal). Signals frame_done on timeline. No timeline wait — the
        binary sem replaces the 5N+4 wait that Path A+ used."""
        if self.phase_c_cmd is None:
            raise RuntimeError("phase_c_cmd not recorded; call prepare_step_cmd_buffers()")
        incoming_sems = list(self.incoming_wait_semaphores.values())
        if incoming_sems:
            wait_value = None
        else:
            # Single-GPU-style sim has no peer; binary sem path is unused.
            # Wait on phase_a_done (5N+1) for queue-order safety only.
            wait_value = self.value_phase_a_done(frame_n)
        self.submit_with_timeline(
            self.phase_c_cmd,
            wait_value=wait_value,
            signal_value=self.value_frame_done(frame_n),
            extra_wait_binary_semaphores=incoming_sems if incoming_sems else None,
        )

    # V3.3: submit_transfer_readback / submit_transfer_upload removed —
    # readback / upload DMAs are now part of phase A / phase C cmd buffers
    # on the compute queue, not separate transfer-queue submissions.
    ...
    def _record_defrag_cmd(self):
        """V3 defrag cmd buffer (mirrors V1 _record_defrag_cmd):
