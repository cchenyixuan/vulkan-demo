"""
renderer_v1.py — V1.0a fork of utils/sph/renderer.py for SphSimulatorV3.
...
Attaches to an SphSimulatorV3 without modifying it.
...
from experiment.v3.utils.camera_v3 import Camera
...
# V3 has its own compiled render SPV (identical shader source as V0/V1).
# parents[1] from experiment/v3/utils/renderer_v3.py = experiment/v3/.
SHADER_RENDER_DIR = (
...
class SphRendererV3:
    ...
    Use:
        sim = SphSimulatorV3(ctx, slab_case, leading_ghost_pool_size=L, ...)
        sim.bootstrap()  (or dual-GPU bootstrap dance)
        viewer = SphRendererV3(sim, window_width=1280, window_height=720)
        viewer.run()
        viewer.destroy()
    """
        ...
        try:
            while not glfw.window_should_close(self.window):
                ...
                if not self.paused and step_fn is not None:
                    # V3: runner-provided step_fn drives the orchestrator
    ...
    def __enter__(self) -> "SphRendererV3":
        return self
