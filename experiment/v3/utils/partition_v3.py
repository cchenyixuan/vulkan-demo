"""
partition_v3.py — V3 dual-GPU static 1D X-axis partition.

Input contract: a **degenerate** CaseV3 from ``case_loader_v3.load_case_v3``
...
Given that input, computes:
  ...
  - per-GPU slab CaseV3    (own column range + leading/trailing ghost +
...
V3 v1.0: 2 GPUs only, static partition (V3+ generalizes to N + dynamic).
"""
...
GHOST_THICKNESS = 1   # V3 v1.0: 1-voxel-thick ghost on the interior side


def _ghost_pool_size(case: CaseV3) -> int:
    """Worst-case pid-slot count for ONE direction's ghost pool.

    V3 v1.0 ghost is 1 voxel thick (``GHOST_THICKNESS``), so the ghost zone
...
def compute_k_split(global_case: CaseV3, weights: list[float]) -> int:
    ...
    """
    if len(weights) != 2:
        raise NotImplementedError("V3 v1.0 supports exactly 2 GPUs")
...
def _build_slab_case(
    ...
    """Build a per-GPU CaseV3 for slot 0 (leftmost) or slot 1 (rightmost).
...
def compute_dual_gpu_partition(
...
) -> tuple[CaseV3, CaseV3, int]:
    """Top-level entry: split global_case into two slab CaseV3 + return K_split.
    ...
    ``pool_safety``:
      ...
      - float (e.g. 1.2): size each slab own_pool_size = ceil(slab_particles *
        ...
        measured peak migrant tail is only ~60-75/defrag-interval, so 1.1-1.2x is
        ample. The install overflow guard + pool_health WARN catch undersizing.
    """
    # Contract check — input must be a degenerate slab from load_case_v3.
    ...
    print(f"[partition_v3] K_split = {k_split} / {grid_nx} "
    ...
    if pool_safety is None:
        ...
        print(f"[partition_v3] pool_safety={pool_safety}: "
