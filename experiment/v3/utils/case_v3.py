"""
case_v3.py — V3 simulation case dataclasses + minimal test factory.

V3 isolation: no import from utils/sph/ or experiment/v1/. The dataclasses
here mirror what V1's Case carries but with the structure flattened for V3's
needs (per-GPU view; orchestrator builds two CaseV3 objects, one per slab).
...
class PhysicsConstants:
    ...
    ``case_loader_v3._calibrate_particle_volume`` to derive each material's
...
class GridLayout:
    ...
    ``partition_v3._build_slab_case`` for the per-slab shift formula.
...
class InitialParticles:
    ...
    material_group: np.ndarray         # (N,) uint32, indexes CaseV3.materials
...
@dataclass
class CaseV3:
    """Per-GPU V3 simulation case. Orchestrator builds one CaseV3 per slab."""
