...
def compile_v3_shaders() -> None:
    ...
    if not os.path.isdir(V2_SHADER_DIR):
        sys.exit(f"V3 shader dir not found: {V2_SHADER_DIR}")
    ...
    for source in sources:
        ...
        if name.startswith("_"):
            print(f"[v3] skip {name} (underscore-prefixed)")
            n_skipped += 1
            continue
    ...
    for source in render_sources:
        ...
        print(f"[v3/render] {name}")
        _run_glslc(source, output)
        n_compiled += 1

    print(f"[v3] compiled {n_compiled} shaders, skipped {n_skipped}")
