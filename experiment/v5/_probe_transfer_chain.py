...
Usage:
    .venv/Scripts/python.exe experiment/v5/_probe_transfer_chain.py \\
...
def main() -> int:
    ...
    ctx_a = VulkanContextV5.create(device_index=args.device_a, application_name="probe_a")
    ctx_b = VulkanContextV5.create(device_index=args.device_b, application_name="probe_b")
    sim_a = SphSimulatorV5(ctx_a, slab0)
    sim_b = SphSimulatorV5(ctx_b, slab1)
    try:
        with DualGpuOrchestratorV5(sim_a, sim_b, defrag_cadence=1000) as orch:
