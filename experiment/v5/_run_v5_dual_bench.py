"""
_run_v5_dual_bench.py — V5 dual-GPU headless BENCHMARK runner.

Same bootstrap as _run_v5_dual.py with no viewer and no debug-log path, plus
...
ticks baked in), then per frame extracts:
  ...
  - worker memcpy µs (already recorded by transport_v5's worker thread)
...
Usage:
    .venv/Scripts/python.exe experiment/v5/_run_v5_dual_bench.py \\
        --case cases/lid_driven_cavity_2d/case.yaml \\
        --max-steps 5000 \\
        --bench-csv logs/bench_001.csv \\
        --bench-window 500

Design note: this runner exists separate from _run_v5_dual.py / _viewer
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="V5 dual-GPU benchmark runner")
    ...
    p.add_argument("--weights", default="3.2,1.0",
                   help="partition weights (W_AMD, W_NV). Default 3.2,1.0 is "
                        "the V5 Path A+ optimum for cross-vendor AMD+NV cavity "
...
def main() -> int:
    ...
    print(f"[bench_v5_dual] case={args.case}")
    print(f"[bench_v5_dual] device_a={args.device_a} device_b={args.device_b} "
          f"weights={weights}  max_steps={args.max_steps}  warmup={args.warmup}")

    global_case = load_case_v5(args.case)
    if args.disable_pst:
        print("[bench_v5_dual] use_pst overridden to False")
    ...
    ctx_a = VulkanContextV5.create(
        ...
        application_name="sph_v5_bench_a")
    ...
    if args.bench_csv:
        ...
        print(f"[bench_v5_dual] CSV → {csv_path}")
    ...
    try:
        with DualGpuOrchestratorV5(sim_a, sim_b,
            ...
            print(f"[bench_v5_dual] {args.max_steps} steps in "
            ...
            print(f"[bench_v5_dual] final: a={s_a['alive_particle_count']:,} "
                  f"b={s_b['alive_particle_count']:,} "
                  f"total={total:,} (expected {expected_total:,})")
            if total != expected_total:
                print(f"[bench_v5_dual] WARN: alive drift = "
                      f"{total - expected_total}", file=sys.stderr)

            # V5 pool-health watermark: own-pool occupancy peak (for data-driven
            ...
            for tag, sim, status in (("AMD/a", sim_a, s_a), ("NV/b", sim_b, s_b)):
                ...
                print(f"[bench_v5_dual] pool_health[{tag}]: "
