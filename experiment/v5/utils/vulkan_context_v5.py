"""
vulkan_context_v5.py — V5-internal Vulkan platform layer.

V5-specific differences vs V0/V1's utils/sph/vulkan_context.py:

  - apiVersion = 1.3  (V0 used 1.2). 1.3 promotes sync2 / vkQueueSubmit2 to
    core, so V5 can call them without enabling VK_KHR_synchronization2 as an
  ...
  - find_memory_type accepts (required, preferred) so HOST_CACHED can degrade
    ...
    cf. docs/sph_v5_design.md §14.5).

  - Per V5 isolation: this module does NOT import utils/sph/. Implementation
...
# V5 uses Vulkan 1.3 to get sync2 + timeline as core (no extension dance).
...
def _find_transfer_queue_family(
    ...
    import sys as _sys
    print(f"[VulkanContextV5] WARNING: no dedicated transfer queue family; "
...
@dataclass
class VulkanContextV5:
    """V5 compute-only Vulkan session handle bundle.

    Use ``VulkanContextV5.create(...)``; destroy via ``destroy()`` or
    context manager.
    ...
    def create(
        ...
        if enable_validation and not validation_supported:
            print(f"[VulkanContextV5] WARNING: validation layer requested but "
        ...
        print(f"[VulkanContextV5] transfer queue family: "
        ...
        # ---- Logical device ---------------------------------------------
        # V5 must enable BOTH timelineSemaphore (1.2 core feature) and
        ...
        # V5 render pipeline (added Phase 5+) doesn't need a feature dance.
    ...
    def __enter__(self) -> "VulkanContextV5":
        return self
