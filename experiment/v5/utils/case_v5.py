"""
case_v5.py — V5 simulation case dataclasses + minimal test factory.

V5 isolation: no import from utils/sph/ or experiment/v1/. The dataclasses
here mirror what V1's Case carries but with the structure flattened for V5's
needs (per-GPU view; orchestrator builds two CaseV5 objects, one per slab).
...
class PhysicsConstants:
    ...
    ``case_loader_v5._calibrate_particle_volume`` to derive each material's
...
class GridLayout:
    ...
    ``partition_v5._build_slab_case`` for the per-slab shift formula.
...
class InitialParticles:
    ...
    material_group: np.ndarray         # (N,) uint32, indexes CaseV5.materials
...
@dataclass
class CaseV5:
    """Per-GPU V5 simulation case. Orchestrator builds one CaseV5 per slab."""
