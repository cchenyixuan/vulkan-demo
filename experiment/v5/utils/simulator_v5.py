"""
simulator_v5.py — V5 per-GPU SPH simulator.
...
class SphSimulatorV5:
    ...
    Lifecycle:
        ctx = VulkanContextV5.create(device_index=...)
        case = load_case_v5("cases/lid_driven_cavity_2d/case.yaml")
        with SphSimulatorV5(ctx, case) as sim:
    ...
    def __enter__(self) -> "SphSimulatorV5":
        return self
    ...
    def _allocate_buffer(
        ...
        (fallback path in VulkanContextV5), CONCURRENT degenerates to a
    ...
    def _allocate_buffers(self) -> dict[str, _Buffer]:
        ...
        for spec in self._buffer_specs:
            ...
            if shared:
                concurrent_count += 1
        print(f"[SimV5] device-local buffers: {len(buffers)}, "
    ...
    def _allocate_scratch_buffers(self) -> dict[str, _Buffer]:
        ...
        for spec in self._buffer_specs:
            ...
            total += spec.size
        print(f"[SimV5] defrag scratch buffers: {len(scratch)}, "
              f"{total / (1024 * 1024):.2f} MB")
        return scratch
    ...
    def _allocate_staging_buffers(self) -> dict[str, _Buffer]:
        ...
        if stagings:
            ...
            print(f"[SimV5] host staging buffers: {len(stagings)}, "
            ...
            print(f"[SimV5] host staging buffers: 0 (single-GPU mode, no peer)")
        return stagings
        ...
        for shader_name in (
            ...
            if not spv_path.exists():
                raise FileNotFoundError(
                    ...
                    f"Run experiment/v5/compile_shaders_v5.py first.")
    ...
    def _upload_initial_state(self) -> None:
        ...
        for buf in self.scratch_buffers.values():
            self._zero_buffer(buf)
        print(f"[SimV5] uploaded initial state ({len(initial)} payload buffers)")
    ...
    def bootstrap_compute(self) -> None:
        ...
        print(f"[SimV5] bootstrap done: alive={status['alive_particle_count']} "
    ...
    def bootstrap(self) -> None:
        ...
        if self._transport_segments:
            raise RuntimeError(
                ...
                "DualGpuOrchestratorV5.bootstrap_all()")
        self.bootstrap_init()
    ...
    def prepare_step_single_cmd_buffer(self) -> None:
        ...
        print(f"[SimV5] step cmd buffer recorded (single-GPU combined)")
    ...
    def submit_step_single_and_wait(self) -> None:
        ...
        sync. Used by _run_v5_single_bench.py and any future single-GPU
        runner."""
