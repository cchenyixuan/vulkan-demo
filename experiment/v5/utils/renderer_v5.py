"""
renderer_v1.py — V1.0a fork of utils/sph/renderer.py for SphSimulatorV5.
...
Attaches to an SphSimulatorV5 without modifying it.
...
from experiment.v5.utils.camera_v5 import Camera
...
# V5 has its own compiled render SPV (identical shader source as V0/V1).
# parents[1] from experiment/v5/utils/renderer_v5.py = experiment/v5/.
SHADER_RENDER_DIR = (
...
class SphRendererV5:
    ...
    Use:
        sim = SphSimulatorV5(ctx, slab_case, leading_ghost_pool_size=L, ...)
        sim.bootstrap()  (or dual-GPU bootstrap dance)
        viewer = SphRendererV5(sim, window_width=1280, window_height=720)
        viewer.run()
        viewer.destroy()
    """
    ...
    def _frame_camera_to_case(self) -> None:
        g = self.case.grid
        h = self.case.physics.smoothing_length
        origin = np.array([g.origin_x, g.origin_y, g.origin_z], dtype=np.float32)
        dim = np.array([g.grid_dimension_x, g.grid_dimension_y, g.grid_dimension_z], dtype=np.float32)
    ...
    def run(self, log_fps_path: Optional[str] = None,
            auto_quit_seconds: Optional[float] = None,
            step_fn=None,
            step_count_fn=None) -> None:
        ...
        try:
            while not glfw.window_should_close(self.window):
                ...
                if not self.paused and step_fn is not None:
                    # V5: runner-provided step_fn drives the orchestrator
                    # (each call = one full 3-submit dual-GPU frame with
                    # synchronous wait_frame_done at the end). After this
                    # returns, particle buffers are ready for render to read.
    ...
    def __enter__(self) -> "SphRendererV5":
        return self
