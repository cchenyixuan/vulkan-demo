"""
_run_v5_single_bench.py — V5 single-GPU headless BENCHMARK runner.
...
threads, one submit + one fence wait per frame. See SphSimulatorV5.
...
Usage:
    .venv/Scripts/python.exe experiment/v5/_run_v5_single_bench.py \\
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="V5 single-GPU benchmark runner")
...
def main() -> int:
    ...
    print(f"[bench_v5_single] case={args.case}")
    print(f"[bench_v5_single] device={args.device}  max_steps={args.max_steps}  "
          f"warmup={args.warmup}")

    # load_case_v5 returns a degenerate CaseV5 with no ghost / no transport
    # (see feedback_loader_partition_decoupling.md — partition_v5 is the
    # sole producer of ghost segments, and we're bypassing it here).
    case = load_case_v5(args.case)
    if args.disable_pst:
        print("[bench_v5_single] use_pst overridden to False")
    ...
    ctx = VulkanContextV5.create(
        ...
        application_name="sph_v5_bench_single")
    ...
    sim.bench = bench
    print(f"[bench_v5_single] timestampPeriod={bench.ns_per_tick:.4f}ns")
    ...
    if args.bench_csv:
        ...
        print(f"[bench_v5_single] CSV → {csv_path}")
    ...
    try:
        ...
        if args.validate_split:
            print(f"[bench_v5_single] VALIDATION MODE: using split pipeline variants")
        ...
        print(f"[bench_v5_single] {args.max_steps} steps in "
        ...
        print(f"[bench_v5_single] final: alive={alive:,} "
              f"(expected {expected_total:,})")
        if alive != expected_total:
            print(f"[bench_v5_single] WARN: alive drift = "
