"""
_run_v5_dual_viewer.py — V5 dual-GPU viewer (GLFW window renders ONE slab).

Drives the full V5 dual-GPU pipeline (3-submit per frame + worker threads +
sync2 timeline + ghost transport) via DualGpuOrchestratorV5; renders the
chosen slab's particles in a GLFW window via SphRendererV5.
...
Usage:
    .venv/Scripts/python.exe experiment/v5/_run_v5_dual_viewer.py [options]

Options:
    --case PATH          case.yaml path
    --device-a/-b N      physical device indices (default 0 / 1)
    --weights W0,W1      partition weights  (default: 3.2,1.0 — V5 Path A+
...
Hotkeys (handled by SphRendererV5): SPACE pause, 0..4 color modes,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="V5 dual-GPU viewer (renders one slab)")
...
def main() -> int:
    ...
    from experiment.v5.utils.debug_log_v5 import DebugLogger
    ...
    print(f"[run_v5_viewer] case={args.case} render_slot={args.render_slot}")
    ...
    print(f"[run_v5_viewer] GLFW required instance extensions: {glfw_exts}")
    ...
    if args.render_slot == 0:
        sim_a = SphSimulatorV5(ctx_render, slab0)
        sim_b = SphSimulatorV5(ctx_compute, slab1)
        render_sim = sim_a
    else:
        sim_a = SphSimulatorV5(ctx_compute, slab0)
        sim_b = SphSimulatorV5(ctx_render, slab1)
        render_sim = sim_b
    ...
    try:
        with DualGpuOrchestratorV5(sim_a, sim_b,
            ...
            try:
                ...
                print(f"[run_v5_viewer] final alive: "
